...
public class Arson extends SkillCard {
    ...
    public String activate(Player nowPlayer) {
        ...
        if ((tempSC != null) && (tempSC.getName().equals("보험카드"))) {
            ...
            BuildingCard tempBC = getTarget().getBc(); // 타겟의 건물 가져옴
            TenantCard tempTC = getTarget().getTc();    // 타겟의 세입자 가져옴
            stringBuilder.append("====== 방화카드 발동 ======\n");
            stringBuilder.append(getTarget().getName() + "에 누군가 불을 지릅니다.\n");
            stringBuilder.append(tempBC.burnBuilding(0.0,nowPlayer));
            if (tempTC != null) {
                stringBuilder.append("건물 전소로 세입자도 떠나갑니다.\n");
                tempTC.setUsable(false);
            }
            stringBuilder.append("===============\n");
        }
